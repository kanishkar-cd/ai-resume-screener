    ...
    <div className="mb-8 border-b border-slate-200 bg-white px-6 py-5">
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="flex gap-2">
          <button type="button" onClick={() => navigate('/dashboard')} className="btn-outline text-[12px] px-3 py-2 cursor-pointer"><ArrowLeft size={13}/> Back to Dashboard</button>
          <button type="button" onClick={() => void editProject()} className="btn-outline text-[12px] px-3 py-2 cursor-pointer"><Pencil size={13}/> Edit Project</button>
        </div>
      </div>
      <div className="flex flex-wrap gap-1 mt-5 -mb-5">
        {tabs.map(([label, suffix]) => {
          const path = `${root}/${suffix}`
          const active = location.pathname === path || (suffix === 'rankings' && (location.pathname === root || location.pathname === `${root}/overview`))
          return <button type="button" key={label} onClick={() => navigate(path)} className={`border-b-2 px-3 py-3 text-[11px] font-semibold cursor-pointer ${active ? 'border-blue-600 text-blue-700' : 'border-transparent text-slate-500 hover:text-slate-900'}`}>{label}</button>
        })}
      </div>
    </div>
  )
