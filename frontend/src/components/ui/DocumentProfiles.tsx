import type { ExtractedJobDescription, ExtractedResume, NormalizedJobDescription, NormalizedResume, ParsedDocument } from '@/api'
...
export function JobProfile({ normalized, extracted, parsed }: { normalized: NormalizedJobDescription; extracted?: ExtractedJobDescription | null; parsed?: ParsedDocument | null }) {
  ...
  return <section className="card p-6 space-y-6">
    <div><p className="text-[10px] font-bold uppercase tracking-widest text-emerald-600">Normalized</p><h2 className="mt-1 text-[20px] font-bold text-slate-900">Final Job Profile</h2><p className="mt-1 text-[12px] text-slate-500">Final backend-normalized data · ruleset {normalized.ruleset_version}</p></div>
...
export function CandidateProfile({ normalized, extracted }: { normalized: NormalizedResume; extracted?: ExtractedResume | null }) {
  const aiRecovery = extracted?.raw_metadata?.ai_recovery === 'merged'
  return <section className="card p-6 space-y-6">
    <div className="flex items-start justify-between gap-4"><div><p className="text-[10px] font-bold uppercase tracking-widest text-emerald-600">Normalized</p><h3 className="mt-1 text-[18px] font-bold text-slate-900">Final Candidate Profile</h3><p className="mt-1 text-[12px] text-slate-500">Final backend-normalized data · ruleset {normalized.ruleset_version}</p></div>{aiRecovery && <span className="rounded-full bg-violet-50 px-2.5 py-1 text-[10px] font-semibold text-violet-700">AI-assisted recovery</span>}</div>
    ...
    {extracted && <details className="border-t border-slate-100 pt-4"><summary className="cursor-pointer text-[12px] font-semibold text-slate-700">View extracted data</summary><div className="mt-4 space-y-5">
      <div className="flex flex-wrap items-center gap-2"><span className="rounded-full bg-slate-100 px-2.5 py-1 text-[10px] font-semibold text-slate-600">Deterministic extraction</span>{aiRecovery && <span className="rounded-full bg-violet-50 px-2.5 py-1 text-[10px] font-semibold text-violet-700">AI-assisted recovery</span>}</div>
