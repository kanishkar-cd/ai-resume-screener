...
export default function Reports() {
  ...
  return <div className="mx-auto max-w-6xl space-y-6">
    ...
    <section className="card p-5"><div className="flex items-center gap-2"><ShieldAlert size={16} className="text-red-600"/><h2 className="text-[16px] font-bold text-slate-900">Rejection Analysis</h2></div><div className="mt-5 grid gap-5 lg:grid-cols-2">
      <div><h3 className="text-[12px] font-bold uppercase tracking-wider text-slate-500">Not Shortlisted Candidates</h3><div className="mt-3 space-y-2">{belowThreshold.length ? belowThreshold.map((item) => <div key={item.document_id} className="rounded-lg border border-amber-200 bg-amber-50 p-3"><div className="flex justify-between"><p className="text-[12px] font-semibold text-slate-800">{item.candidate_name}</p><p className="text-[13px] font-bold text-amber-800">{item.final_score}</p></div><p className="mt-1 text-[11px] text-amber-800">Candidate does not meet component decision shortlisting rules.</p></div>) : <p className="text-[12px] text-slate-400">No eligible candidates were rejected by score.</p>}</div></div>
