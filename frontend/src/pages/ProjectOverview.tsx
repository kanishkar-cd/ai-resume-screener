...
export default function ProjectOverview() {
  ...
  const stages=[['Job Description',jd],['Resumes',resumeCount>0],['Weightage',state.weightConfigSaved],['Processing',state.jdNormalized&&resumesReady],['Ranking',ranked],['Reports',ranked]] as const
  const completed=stages.filter(([,done])=>done).length; const progress=Math.round(completed/stages.length*100)
  const next=!jd?'job-description':resumeCount===0?'resumes':!state.weightConfigSaved?'weightage':!ranked?'rankings':'reports'
  const nextLabel=!jd?'Upload job description':resumeCount===0?'Upload resumes':!state.weightConfigSaved?'Set scoring criteria':!ranked?'Run candidate scoring':'View reports'
