...
class ComponentScoringService:
    ...
    def score(self, resume: Any, job: Any, config: Any, projects: list[dict[str, Any]] | None = None) -> ComponentScores:
        ...
        required_skills = list(dict.fromkeys([*(job.skills or []), *(config.mandatory_skills or [])]))
    ...
    def _match(self, candidate: list[str], required: list[str], label: str) -> ComponentScoreDetail:
        ...
        matched = [item for item in required if item.casefold() in candidate_keys]
        missing = [item for item in required if item.casefold() not in candidate_keys]
