...
def test_component_engine_formulas_and_boundaries() -> None:
    ...
    assert scores.languages.score == 0.0


def test_no_requirement_component_scores_zero() -> None:
    ...
    # Absent/unconfigured categories score 0.0
    assert all(getattr(scores, name).score == 0.0 for name in ComponentScores.model_fields)
...
def test_ai_relevance_with_all_some_and_no_applicable_categories() -> None:
    """Test Core component model with all categories, some N/A, and all N/A categories."""
    from app.services.scoring.weight_calculation_service import WeightCalculationService
    from app.schemas.scoring import ComponentScoreDetail, ComponentScores
    ...
    # 1. Active core categories: skills=100 (45%), resp=100 (40%), pref=100 (15%) -> 100.0
    all_categories = {"skills", "responsibilities", "preferred_skills", "experience", "education", "certifications"}
    ...
    # 2. Some categories: skills (100 * 45/85 = 52.94) and resp (100 * 40/85 = 47.06) = 100.0
    some_categories = {"skills", "responsibilities"}
    score_some = WeightCalculationService.final_score(0, 0, 0, components=components, applicable_categories=some_categories)
    assert score_some == 100.0

    # 3. Only skills: 100 * (45/45) = 100.0
...
def test_preferred_skill_bonus_reaches_final_score_and_caps_at_100() -> None:
    ...
    # Skills (80 * 0.45 = 36.0), Responsibilities (80 * 0.40 = 32.0), Preferred (100 * 0.15 = 15.0) -> 83.0
    ...
    final_100 = WeightCalculationService.final_score(
        weighted_total=100.0, penalty_total=0.0, bonus_total=4.0, components=components_max,
        applicable_categories={"skills", "responsibilities", "projects", "preferred_skills"},
    )
