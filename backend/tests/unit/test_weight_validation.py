...
def test_invalid_weight_total_is_rejected(skills: float) -> None:
    payload = WeightConfigCreate(weights=WeightDistribution(skills=skills))
    with pytest.raises(InvalidWeightTotalException):
        WeightConfigService._validate(payload)
...
class StubConfigs:
    ...
    async def create_or_update(self, project_id, payload):
        if self.model is None:
            self.model = SimpleNamespace(id=uuid4(), project_id=project_id, version=1, created_at=__import__("datetime").datetime.now(__import__("datetime").UTC))
        else: self.model.version += 1
        self.model.updated_at = self.model.created_at
        for name, value in payload.weights.model_dump().items(): setattr(self.model, f"{name}_weight", value)
        for name, value in payload.model_dump().items():
            if name != "weights": setattr(self.model, name, value)
        return self.model
...
async def test_partial_patch_preserves_values_and_increments_version() -> None:
    configs = StubConfigs()
    service = WeightConfigService(StubProjects(), configs)
    project_id = uuid4()
