...
def test_10_experience_requirement_with_insufficient_experience():
    ...
    matcher = DeterministicRequirementMatcher()
    verdict = matcher.match(req, resume, evidence)
    assert verdict.status in {MatchStatus.UNRESOLVED, MatchStatus.PARTIALLY_MATCHED}
    assert "below required" in verdict.reasoning or "months vs" in verdict.reasoning or verdict.coverage < 1.0
